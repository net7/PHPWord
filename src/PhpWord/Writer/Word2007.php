...
use Net7\PhpWord\Element\Section;
use Net7\PhpWord\Media;
...
class Word2007 extends AbstractWriter implements WriterInterface
{
    ...
    public function __construct(PhpWord $phpWord = null)
    {
        ...
        foreach (array_keys($this->parts) as $partName) {
            ...
            if (class_exists($partClass)) {
                /** @var \Net7\PhpWord\Writer\Word2007\Part\AbstractPart $part Type hint */
                $part = new $partClass();
     ...
     *
     * @param \Net7\PhpWord\Shared\ZipArchive $zip
        ...
        if (!empty($elements)) {
            foreach ($elements as $file => $media) {
                if (count($media) > 0) {
                    ...
                    /** @var \Net7\PhpWord\Writer\Word2007\Part\AbstractPart $writerPart Type hint */
     ...
     *
     * @param \Net7\PhpWord\Element\Section &$section
     * @param \Net7\PhpWord\Shared\ZipArchive $zip
    ...
    private function addHeaderFooterContent(Section &$section, ZipArchive $zip, $elmType, &$rId)
    {
        ...
        /** @var \Net7\PhpWord\Element\AbstractElement $element Type hint */
    ...
    private function addNotes(ZipArchive $zip, &$rId, $noteType = 'footnote')
    {
        ...
        /** @var \Net7\PhpWord\Collection\AbstractCollection $collection Type hint */
    ...
    private function addChart(ZipArchive $zip, &$rId)
    {
        ...
        $index = 0;
        if ($collection->countItems() > 0) {
            /** @var \Net7\PhpWord\Element\Chart $chart */
